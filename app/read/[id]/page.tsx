...
import { Comic, Panel, Bubble } from '../../../types'
...
export default function ComicReader() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Draw caption overlay at top of a panel
      const drawCaption = (text: string, x: number, y: number, w: number) => {
        pdf.setFontSize(10)
        const maxW = Math.min(w - 6, w * 0.88)
        const lines = pdf.splitTextToSize(text.toUpperCase(), maxW)
        const lineH = 4.5
        const capH = Math.max(10, lines.length * lineH + 4)
        pdf.setFillColor(255, 255, 255)
        pdf.rect(x + 1.5, y + 1.5, maxW + 4, capH, 'F')
        pdf.setDrawColor(20, 20, 20)
        pdf.setLineWidth(0.4)
        pdf.rect(x + 1.5, y + 1.5, maxW + 4, capH, 'S')
        pdf.setTextColor(20, 20, 20)
        pdf.text(lines.slice(0, 3).join('\n'), x + 3.5, y + 6.5)
      }
